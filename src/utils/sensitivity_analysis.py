...
class Base:
    ...
    def _my_gen_masks(
        ...
        print(self.gen_mask)
        print(f'mask video size before: {video_size}')
        video_size = torch.Size([video_size[0], video_size[2], video_size[1], video_size[3], video_size[4]])
        print(f'mask video size after: {video_size}')
        ...
        if self.gen_mask == "simple":
            ...
            m = m.squeeze(1)
            print(f'm shape after squeeze: {m.shape}')
            fxfy = np.array(fxfy)

        else:
            assert False, "gen_mask"
        print(self.N_stack_mask)
        if self.N_stack_mask != 1:
            m = self._stack_mask_method(m, fxfy)  # 0.27s
        return m
    ...
    def _my_normalize(
        ...
        print(f'N: {self.N}')
        _map = _X @ (_mask.reshape(self.N, -1) / _mask.reshape(_mask.shape[0], -1).mean(1)[:, np.newaxis])
        new_video_size = torch.Size([self.video_size[0], self.video_size[2], self.video_size[1], self.video_size[3], self.video_size[4]])
        print(f'video size: {new_video_size}')
        map = _map.cpu().view(new_video_size[1:]).mean(0) / self.N
        print(f'map shape: {map.shape}')
    ...
    def _my_forward(self, x, requires_grad=False, target_class=None):
        if x.dim() == 4:
            x = x.unsqueeze(0)
        else:
        ...
        N = x.shape[0]
        # _probs = [self.net(x[j : min([j + bs, N]), ...]) for j in range(0, N, bs)]
        # _probs = [prob[0] if isinstance(prob, tuple) else prob for prob in _probs]

        with torch.no_grad():
            _probs = []
            for j in range(0, N, bs):
                output = self.net(x[j : min([j + bs, N]), ...])
                if isinstance(output, tuple):
                    _probs.append(output[0])
                else:
                    _probs.append(output)
    ...
    def _my_run(
        ...
        # self.video_size = torch.Size([1, 16, 3, 224, 224])
        self.N = self.masks.shape[0]
        ...
        if get_feat:
            _probs = self._my_forward(inputs)
        else:
            # こっちが呼ばれる
            _probs = self._my_forward(inputs, target_class=target_class)
        print(f'probs shape: {_probs.shape}')
        results = self._post_process(org_prob, _probs)
        return results
...
class MyOcclusionSensitivityMap3D(Base):
    def __init__(self, *args, **kargs):
        super().__init__(*args, **kargs)

    def _post_process(self, org_prob, _probs):
        return self._my_normalize(_probs, org_prob)

    def run(self, org_video, target_class):
        with torch.inference_mode():
            org_tensor = org_video.clone()
            org_tensor = org_tensor.permute(1, 0, 2, 3)
            trans_video = []
            for i in range(self.video_size[1]):
                img = org_tensor.squeeze().transpose(0, 1)[i]
                trans_video.append(self.unnormalize(img))
            # trans_video = torch.stack(trans_video)
            org_feat = self._my_forward(org_video, target_class=target_class)
            print(f'org_feat shape: {org_feat.shape}')
            print(f'org_tensor shape: {org_tensor.shape}')
            print(f'trans_video length: {len(trans_video)}')
            results = self._my_run(org_tensor, org_feat, target_class, trans_video)
