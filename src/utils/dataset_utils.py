...
def load_jpg_my_data(l, n, inference_class_names, transform, file):
    name = inference_class_names[l]
    dir = os.path.join(
        "/home/kazuoki/AOSA2/right-handed_images/" + name + "/" + file
        )
    path = sorted(glob.glob(dir + "/*"), key=numericalSort)
    ...
    # target_path = []
    # for i in range(16):
    #     target_path.append(path[length * i // 16])
    # print(target_path)
...
def load_jpg_my_eco(l, n, inference_class_names, transform):
    name = inference_class_names[l]
    dir = os.path.join(
        "/home/kazuoki/AOSA2/myECO/data/left-handed/" + name + "/L4001"
        )
    path = sorted(glob.glob(dir + "/*"), key=numericalSort)
    ...
    video = transform(target_path, 'val')
    # for _p in target_path:
    #     video.append(transform(Image.open(_p), 'val'))
